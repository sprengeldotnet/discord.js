...
client.on('ready', () => {
    console.log('ready');
    client.user.setActivity('anime | .help', { type: 'WATCHING' });
});
    ...
    if(message.content.startsWith(".")){
        ...
        if(command=="meme"){
            ...
            https.get('https://host.io/api/full/'+z+'?token=TOKEN', (res)=> {
                ...
                res.on('end', () => {
                    var r = JSON.parse(data);
                    if(r['dns']['domain']==null) return message.channel.stopTyping(); message.channel.send("could not find anything. sowwy");
                    var _a = '\r\n';
                    if(r['dns']['a'][0]!=null){
                        r['dns']['a'].forEach(ip => {
                            if(_a=='\r\n'){
